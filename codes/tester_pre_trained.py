...
class Dataloader_trdp_pre_trained(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if self.transform is not None:
            ...
            print(f"1-{sample['raster_diff'].shape} - {sample['mask_diff'].shape}")
            raster1 = np.moveaxis(np.uint8(sample['raster_diff'][:3]),0,-1)
            mask = np.moveaxis(np.uint8(sample['mask_diff']),0,-1)
            
            print(f"2-{sample['raster_diff'].shape} - {sample['mask_diff'].shape}")
            import random
            ...
            print(f"3-{sample['raster_diff'].shape} - {sample['mask_diff'].shape}")
            set_seed(seed)
